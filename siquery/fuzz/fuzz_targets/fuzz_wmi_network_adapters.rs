...
use siquery::tables::{WmiNetworkAdaptersIface,WmiNetworkAdapters};
...
cfg_if! {
    if #[cfg(target_os = "linux")] {
        impl WmiNetworkAdaptersIface for StringFuzzer {
...
fuzz_target!(|str_data: &[u8]| {
    WmiNetworkAdapters::get_specific_ex(&StringFuzzer::new_str(str_data));
});
